...
class BlendHandoverPlotter(BasePlotter):
    ...
    def _plot_handover_curve(self, ood_rows: list, output_dir: Path, clean_exp: str):
        ...
        axes[0].plot(
            ...
            markersize=7,
            linewidth=3.0,
            label="Logic Policy ($w_{\\mathrm{logic}}$)",
            zorder=5,
        )
        axes[0].fill_between(
            x_ticks, logic_means - logic_sems, logic_means + logic_sems, color="#d95f02", alpha=0.20, zorder=4
        )
        ...
        axes[0].plot(
            ...
            markersize=7,
            linewidth=3.0,
            label="Neural Policy ($w_{\\mathrm{neural}}$)",
            zorder=5,
        )
        axes[0].fill_between(
            x_ticks, neural_means - neural_sems, neural_means + neural_sems, color="#2b5c8f", alpha=0.20, zorder=4
        )
        ...
        for m_name in df["Model"].unique():
            ...
            axes[0].plot(
                x_ticks,
                m_df["Mean_Logic_Weight"].values * 100.0,
                color="#d95f02",
                alpha=0.20,
                linestyle="--",
                linewidth=1.0,
            ...
            axes[0].plot(
                x_ticks,
                m_df["Mean_Neural_Weight"].values * 100.0,
                color="#2b5c8f",
                alpha=0.20,
                linestyle="--",
                linewidth=1.0,
        ...
        axes[0].set_title(
            "A. Mean Authority: Neural Optimization ➔ Symbolic Safety Fallback", fontsize=11, fontweight="bold", pad=10
        )
        ...
        axes[0].set_xlabel(
            "Epistemic Uncertainty Quantiles (k-NN Distance from Training Support)\n← Familiar Training Distribution    |    Unseen / Outlier States →",
            fontsize=9.5,
            fontweight="bold",
        )
        ...
        axes[1].plot(
            ...
            markersize=7,
            linewidth=2.8,
            label="Pure Logic Decisions (≥ 90% Authority)",
        )
        axes[1].fill_between(
            x_ticks, pure_log_means - pure_log_sems, pure_log_means + pure_log_sems, color="#d95f02", alpha=0.20
        )
        ...
        axes[1].plot(
            ...
            markersize=7,
            linewidth=2.8,
            label="Pure Neural Decisions (≥ 90% Authority)",
        )
        axes[1].fill_between(
            x_ticks, pure_neu_means - pure_neu_sems, pure_neu_means + pure_neu_sems, color="#2b5c8f", alpha=0.20
        )

        axes[1].set_title(
            "B. Decisive Commitment: Transitions Handed Completely to One Policy",
            fontsize=11,
            fontweight="bold",
            pad=10,
        )
        ...
        fig.suptitle(
            f"BlendRL Neuro-Symbolic Safety Fallback: Policy Handover across Out-of-Distribution Regimes ({clean_exp})",
            fontsize=12.5,
            fontweight="bold",
            y=0.98,
        )
