...
class Renderer:
    ...
    def __init__(
        ...
        self.model._print() # This prints to terminal
        ...
        try:
            ...
            if not hasattr(self, "action_meanings"):
                self.action_meanings = None
            self.keys2actions = {}
            
        self.current_keys_down = set()

        if hasattr(self.model, "actor") and hasattr(self.model.actor, "logic_actor"):
            self.predicates = self.model.actor.logic_actor.prednames
        else:
            self.predicates = []
    ...
    def _init_pygame(self):
        ...
        # frame is (H, W, C), Pygame wants (W, H)
        h, w = frame.shape[:2]
        self.env_render_shape = (w, h)
        
        window_width = w
        if self.render_predicate_probs:
            window_width += PREDICATE_PROBS_COL_WIDTH
            
        self.window = pygame.display.set_mode((window_width, h), pygame.SCALED)
    ...
    def run(self):
        ...
        obs_nn = obs_nn.detach().clone().to(self.model.device)
        ...
        while self.running:
            ...
            if not self.paused:
                ...
                new_obs_nn = new_obs_nn.detach().clone().to(self.model.device)
                ret += float(reward)
                ...
                if self.reset or done or terminations:
                    print(f"Return: {ret:.2f} - Length {length}")
                    ret = 0
                    length = 0
                    obs, obs_nn = self.env.reset()
                    obs_nn = obs_nn.detach().clone().to(self.model.device)
    ...
    def _render_env(self):
        frame = self.env.env.render()
        # Transpose from (H, W, C) to (W, H, C) because surfarray expects (Width, Height)
    ...
    def _render_policy_probs(self):
        ...
        if not hasattr(self.model, "get_policy_weights"):
            return
    ...
    def _render_predicate_probs(self):
        anchor = (self.env_render_shape[0] + 10, 25)
        if not hasattr(self.model, "actor") or not hasattr(self.model.actor, "logic_actor"):
            return
    ...
    def _render_neural_probs(self):
        anchor = (self.env_render_shape[0] + 10, 25)
        if not hasattr(self.model, "actor") or not hasattr(self.model.actor, "neural_action_probs"):
            return
        ...
        if self.action_meanings is not None:
            ...
            action_names = [f"action_{i}" for i in range(len(action_vals))]
