...
class BlenderActor(nn.Module):
    ...
    def to_blender_policy_distribution(self, neural_state, logic_state):
        ...
        for i in range(2): # 0: neural, 1: logic
            indices = torch.tensor(self.blender_id_to_pred_indices[i], device=self.device)
...
class BlenderActorCritic(nn.Module):
    ...
    def __init__(
        ...
        if self.env.name == "mountaincar":
            self.visual_neural_actor = module.MLP(device=device, out_size=self.env.n_actions)
        else:
