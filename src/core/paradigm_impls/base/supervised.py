...
class SupervisedRunner(BaseParadigmRunner):
    ...
    def run(
        ...
        """Standard execution loop for single-model supervised experiments."""
        epochs = int(cfg.get("epochs", 20))
        model = context.get("model")
        if model is None:
            raise ValueError(
                "SupervisedRunner requires a model in context or a registered agent class. "
                "For task-specific pipelines (e.g. early prediction), use task handlers."
            )

        train_loader = data_module.train_dataloader()
        val_loader = data_module.val_dataloader()

        self.train_model(model, train_loader, val_loader=val_loader, epochs=epochs, callbacks=callbacks)

        if val_loader is not None and eval_protocol is not None:
            metrics = eval_protocol.evaluate(model, val_loader, cfg=cfg)
