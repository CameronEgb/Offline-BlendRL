...
class InferModule(nn.Module):
    ...
    def __init__(self, I_tensor, m, infer_step, gamma=0.01, device=None, train=False):
        """
        ...
        # clause functions
        self.cs = nn.ModuleList([ClauseFunction(i, self.I[i], gamma=gamma)
...
class ClauseBodyInferModule(nn.Module):
    ...
    def __init__(self, I_tensor, gamma=0.01, device=None, train=False):
        """
        In the constructor we instantiate two nn.Linear modules and assign them as
        member variables.
        """
        ...
        # clause functions
        self.cs_bs = nn.ModuleList([ClauseBodySumFunction(i, self.I[i], gamma=gamma)
                                     for i in range(self.I.size(0))])
        self.cs = nn.ModuleList([ClauseFunction(i, self.I[i], gamma=gamma)
                                 for i in range(self.I.size(0))])
...
class ClauseFunction(nn.Module):
    ...
    def __init__(self, i, I_tensor, gamma=0.01):
        ...
        self.S = self.I.size(-2)  # max number of possible substitutions
        self.gamma = gamma

    def forward(self, x):
        # x: B * G
        # self.I: G * S * L
        # Use advanced indexing for efficiency and to avoid MPS repeat/gather issues
        V_indexed = x[:, self.I]  # (B, G, S, L)

        # prod over body atoms (L)
        # (B, G, S)
        C_subs = torch.prod(V_indexed, dim=3)

        # softor over substitutions (S)
        # (B, G)
        C = softor(C_subs, dim=2, gamma=self.gamma)
        return C
...
class ClauseBodySumFunction(nn.Module):
    ...
    def forward(self, x):
        ...
        # prod over body atoms (L), then sum over substitutions (S)
        # (B, G)
        C = torch.sum(torch.prod(V_indexed, dim=3), dim=2)
        return C
