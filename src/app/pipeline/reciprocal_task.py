...
Usage:
  ...
  or via: python -m src.app.pipeline.reciprocal_task --config <experiment.yaml>
...
def run_reciprocal_refinement(cfg, local_val):
    ...
    print(f"\n{'=' * 70}")
    print("  RECIPROCAL REFINEMENT: Iterative EP ↔ CQL Co-Training")
    ...
    for round_k in range(max_rounds + 1):  # Round 0 is bootstrap
        ...
        if methods and hasattr(methods, "items"):
            ...
            cql_model = mcfg.get("model", "dnn")
        else:
            cql_method_name = "cql_dnn"
            cql_agent = "cql"
            cql_model = "dnn"
        ...
        if local_val:
            ...
            print("  [Slurm mode not yet implemented for reciprocal refinement]")
            return
        ...
        ep_cmd = [
            python_exe,
            "-u",
            "src/usr/eval/early_prediction/model.py",
            "--exp-id",
            f"{exp_id}_round{round_k}",
            "--checkpoint",
            str(round_ckpt_dir),
            "--save-checkpoints",
        ]
        ...
        if local_val:
            ...
            try:
                eval_results = compute_ep_eval_data(
                    checkpoint_root=str(round_ckpt_dir), dataset_path=None, ep_ckpt_root=str(round_ep_ckpt_dir)
                )
        ...
        if round_k >= 1 and len(round_metrics) >= 2:
            ...
            delta_mort = abs(
                curr_metrics.get("counterfactual_mortality", 0) - prev_metrics.get("counterfactual_mortality", 0)
            )
    ...
    # ==================================================================
    print(f"\n{'=' * 60}")
    print("  Generating convergence summary plots...")
    print(f"{'=' * 60}")
...
def _copy_best_checkpoint(cfg, round_exp_id, dest_dir):
    ...
    for search_dir in search_dirs:
        if search_dir.exists():
            ckpts = sorted(search_dir.rglob("best_model*.ckpt"), key=lambda p: p.stat().st_mtime, reverse=True)
            if not ckpts:
                ckpts = sorted(search_dir.rglob("*.ckpt"), key=lambda p: p.stat().st_mtime, reverse=True)
            if ckpts:
...
def _plot_convergence(round_metrics, output_dir):
    ...
    plt.suptitle("Reciprocal Refinement — Convergence Across Rounds", fontsize=15, fontweight="bold", y=1.02)
    plt.tight_layout()
