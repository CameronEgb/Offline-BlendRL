...
def get_task(task_name: str):
    ...
    matches = [
        (prefix, fn)
        for prefix, fn in TASK_REGISTRY.items()
        if task_name == prefix or task_name.startswith(prefix + "_")
    ]

    if not matches:
        registered = sorted(TASK_REGISTRY.keys())
        raise ValueError(f"Unknown task: '{task_name}'. Registered prefixes: {registered}")
...
def auto_discover_tasks():
    ...
    for module_info in pkgutil.iter_modules([pipeline_dir]):
        if module_info.name.startswith("_") or module_info.name == "task_registry":
            continue
        try:
            importlib.import_module(f"src.app.pipeline.{module_info.name}")
            ...
            logger.warning("Error importing src.app.pipeline.%s: %s", module_info.name, e)
