...
def get_terms_by_dtype_and_depth(dtype, depth, terms, max_term_depth):
    return [term for term in terms if term.dtype == dtype and term.max_depth() + depth <= max_term_depth]
...
def generate_substitutions(atoms, terms, max_term_depth):
    ...
    assert not invalid_var_dtypes(var_dtype_list), "Invalid data type to generate substitutions for existentially quantified variables."
