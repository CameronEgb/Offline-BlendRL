...
def objective(trial, seq_data_dict, y_cohort, c_indices, args):
    ...
    split_scores = []
    n_eval_splits = getattr(
        args, "n_eval_splits", 5
    )  # 5 stratified cross-validation splits per trial for robust evaluation
...
def main(cfg: DictConfig):
    ...
    seq_data_dict = {"no_v": seq_data_base, "with_v": seq_data_with_v}
    ...
    try:
        ...
        ax.plot(
            ...
            color="tab:red",
            linewidth=2.5,
            label=f"Best Cumulative {args.metric.upper()}",
        )
