...
from src.core.interfaces import BaseDataModule
from src.early_prediction.model import compute_volatility_features, normalize_features
...
class EPSepsisDataModule(BaseDataModule):
    ...
    def setup(self, cfg: Any) -> None:
        ...
        dataset_path = ep_cfg.get("dataset_path")
        if not dataset_path:
            raise ValueError(
                "early_prediction.dataset_path is required. "
                "Add 'dataset_path: in/datasets/mimic/<file>.npz' to the experiment config."
            )
    ...
    def train_dataloader(self) -> DataLoader:
        loader, _, _, _ = self.get_train_dataloader(split_idx=0)
        return loader
